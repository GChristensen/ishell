...
test:
	make commands
	cd addon; sh start web-ext run -p "$(FIREFOX_PROFILES)/debug.ishell" --keep-profile-changes

test-nightly:
	make commands
	cd addon; sh start web-ext run -p "$(FIREFOX_PROFILES)/debug.ishell.nightly" --firefox=nightly --keep-profile-changes
