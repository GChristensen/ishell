...
Utils.easterListener = function(input) {
    if (input.trim().toLowerCase() === "enable debug mode") {
        shellSettings.debug_mode(true, () => chrome.runtime.reload());
        return true;
    }
    else if (input.trim().toLowerCase() === "disable debug mode") {
        shellSettings.debug_mode(false, () => chrome.runtime.reload());
        return true;
    }

    if (input.trim().toLowerCase() === "enable more commands") {
        shellSettings.enable_more_commands(true, () => chrome.runtime.reload());
        return true;
    }
    else if (input.trim().toLowerCase() === "disable more commands") {
        shellSettings.enable_more_commands(false, () => chrome.runtime.reload());
        return true;
    }
