...
//                           icons inlined. The animated demo stays a file (it has to be a separate
//                           document to load web fonts), minified into media/landing/.
...
if (plain) {
  ...
  // svgo's structural plugins don't understand animation: removeHiddenElems deletes elements that are
  // opacity:0 until their keyframes bring them in, and group/shape rewrites drop the class names the
  // keyframes are attached to (the typing stops). So an animated SVG -- CSS keyframes (the demo,
  // generated by tools/gen-demo.mjs) or SMIL -- gets only cosmetic plugins that never restructure
  // elements. Static SVGs get the full default preset.
  ...
  const svgOpts = (src) => ({
    multipass: true,
    plugins: /<(animate|set)\b|@keyframes/.test(src)
      ? CAREFUL
